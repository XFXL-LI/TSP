#include "Pm10Collect.h"
#include "../../../../inc/sys_init.h"
#include "../../../../module/log/log_manager.h"
#include "../../../../module/Serial/SerialManager.h"
#include "../../collectorManager.h"
static CollectorRegistrar _registrar_pm10("a34002", &Pm10Collect::getInstance());

Pm10Collect *Pm10Collect::_instance = nullptr;

Pm10Collect &Pm10Collect::getInstance()
{
    if (_instance == nullptr)
    {
        _instance = new Pm10Collect();
    }
    return *_instance;
}

Pm10Collect::Pm10Collect()
...
bool Pm10Collect::gal(int increment, int ratio) {
    ...
    if (xSemaphoreTake(_StreamTTLMutex, pdMS_TO_TICKS(3000)) == pdTRUE) {
        ...
        writeVal[1] = ratio;
        uint16_t raw[2] = {0};
        if (!_mb_manager->writeModbusRegs(1, 0x34, 2, writeVal)) {
        ...
        if (_mb_manager->readModbusRegs(1, 0x34, 2, raw)) {
...
String Pm10Collect::getID() const
{
    return _id;
}

DataPacket *Pm10Collect::collect()
{
...
void Pm10Collect::setFactor(float factor)
{
    _factor = factor;
}

bool Pm10Collect::updatePort(Stream *new_port)
{
    if (new_port == nullptr)
        return false;
    _port = new_port;
...
void Pm10Collect::setIdentity(String id, int group)
{
    _id = id;
}
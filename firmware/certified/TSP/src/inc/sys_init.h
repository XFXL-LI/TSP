...
// HJ 212 factor data status. Keep validity separate: is_valid controls local
// statistics, while status describes why a value is normal or unavailable.
enum class DataStatus : char {
    NORMAL = 'N',
    OUT_OF_SERVICE = 'F',
    MAINTENANCE = 'M',
    MANUAL = 'S',
    ...
    CALIBRATION = 'C',
    OVER_RANGE = 'T',
    COMMUNICATION_FAILURE = 'B'
};

inline char dataStatusCode(DataStatus status) {
    return static_cast<char>(status);
}

inline uint8_t dataStatusPriority(DataStatus status) {
    switch (status) {
        case DataStatus::MAINTENANCE:          return 7;
        case DataStatus::CALIBRATION:          return 6;
        case DataStatus::COMMUNICATION_FAILURE:return 5;
        case DataStatus::SENSOR_FAULT:         return 4;
        case DataStatus::OVER_RANGE:           return 3;
        case DataStatus::OUT_OF_SERVICE:       return 2;
        case DataStatus::MANUAL:               return 1;
...
struct DataPacket {
    ...
    DataPacket()
        : value(0.0f), is_valid(false),
          status(DataStatus::COMMUNICATION_FAILURE), refCount(1) {
...
struct ProcessedDataPacket {
    ...
    float cou_val = 0.0f;
    bool is_valid = false;
    DataStatus status = DataStatus::COMMUNICATION_FAILURE;
};
...
struct configData{
    ...
    String message;
    std::atomic<int> refCount;
    configData() : success(true), refCount(1) {} 
    void retain() {
        refCount.fetch_add(1, std::memory_order_relaxed);
    }
    void release() {

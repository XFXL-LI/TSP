...
namespace {
constexpr const char *CONFIG_PATH = "/gasCalibrationCertified.json";
constexpr const char *CONFIG_PROFILE = "certified_250ppb_v1";
constexpr const char *DEFAULT_CONFIG_JSON =
    "{\"profile\":\"certified_250ppb_v1\","
    "\"span_targets_ppb\":{\"w34011\":250,\"a21004\":250,"
    "\"a21005\":5000,\"a21026\":250},\"max_deviation_percent\":20,"
...
bool GasCalibrationManager::loadConfiguration()
{
    ...
    LOG_INFO("GAS_CAL_CONFIG profile=%s path=%s targets_ppb=%u,%u,%u,%u deviation=%u stability=%u",
...
bool GasCalibrationManager::parseConfiguration(const String &json)
{
    ...
    bool valid = cJSON_IsString(profile) && profile->valuestring != nullptr &&
                 strcmp(profile->valuestring, CONFIG_PROFILE) == 0;

    cJSON *targets = cJSON_GetObjectItemCaseSensitive(root, "span_targets_ppb");
    uint16_t parsedTargets[PROFILE_COUNT] = {};
    valid = valid && cJSON_IsObject(targets);
...
void GasCalibrationManager::tick()
{
    ...
    if (doConcentrationRead) {
        uint16_t rawConcentration = 0;
        IoResult io = readRegister(
            PROFILES[profileIndex].slave, 0x6001, rawConcentration);
        const uint16_t concentration = GasSpecificPolicy::applyPpbLimit(
            PROFILES[profileIndex].id, rawConcentration);
        if (xSemaphoreTake(_stateMutex, pdMS_TO_TICKS(50)) == pdTRUE) {
            if (_session == session &&
                ...
                if (io == IoResult::DEFERRED) {
                    ...
                    LOG_DEBUG("GAS_CAL_READ id=%s raw_ppb=%u value_ppb=%u ok=1",
                               PROFILES[_profileIndex].id, rawConcentration,
                               concentration);
                } else {
                    _busWaiting = false;

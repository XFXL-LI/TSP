#include "MeteCollect.h"
#include "../../../../inc/sys_init.h"
#include "../../../../module/log/log_manager.h"
#include "../../../../module/Serial/SerialManager.h"
#include "../../collectorManager.h"
#include "../SensorReadPolicy.h"
static CollectorRegistrar _registrar_mete("a01002", &MeteCollect::getInstance());

MeteCollect* MeteCollect::_instance = nullptr;

MeteCollect& MeteCollect::getInstance() {
    if (_instance == nullptr) {
        _instance = new MeteCollect();
    }
    return *_instance;
}

MeteCollect::MeteCollect() 
...
bool MeteCollect::modbusInit(Stream* new_port, String id, String port_name, float factor, String unit) { 
...
bool MeteCollect::gal(int increment, int ratio) {
    (void)increment;
    (void)ratio;
    return true;
}

String MeteCollect::getID() const { 
    return _id; 
}

DataPacket* MeteCollect::collect() {
    ...
    if (xSemaphoreTake(_StreamMutex, pdMS_TO_TICKS(3000)) == pdTRUE)
    {
        ...
        {
            const float value = static_cast<float>(currentVal) / 10.0f;
            packet->value =
        ...
        {
            LOG_WARNING("Humidity read failed for %s", _id.c_str());
        }
    }
    else
    {
...
void MeteCollect::setModbusConfig(uint8_t slave, uint16_t reg, uint8_t count, float factor) {
    _slaveId = slave;
    _regAddr = reg;
        _regCount = count;
    _factor = factor;
...
void MeteCollect::setFactor(float factor) { 
    _factor = factor; 
}

bool MeteCollect::updatePort(Stream* new_port) {
    if (new_port == nullptr) return false;
    _port = new_port;
    if (_mb_manager) {
        _mb_manager->modbus_init(_port);
...
void MeteCollect::setIdentity(String id, int group) {
    _id = id;

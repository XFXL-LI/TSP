...
#include "../../../../module/gas/GasSpecificPolicy.h"
...
DataPacket* So2Collect::collect() {
    ...
    const uint16_t outputPpb = GasSpecificPolicy::applyPpbLimit(
        _id.c_str(), gasRegisters[1]);
    packet->value = static_cast<float>(outputPpb);
    packet->status = GasStatusPolicy::classify(gasRegisters[0]);
    packet->is_valid = packet->status == DataStatus::NORMAL;
    LOG_DEBUG("K-7S SO2 status=0x%04X raw=%u value=%u ppb flag=%c",
              gasRegisters[0], gasRegisters[1], outputPpb,
              dataStatusCode(packet->status));
    return packet;
}

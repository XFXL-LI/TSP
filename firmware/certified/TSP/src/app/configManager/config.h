...
#define SENSOR_CATALOG_PATH "/sensorCatalog.json"
...
private:
    ...
    void getSensorSelectionRes(configData *resData);
    void getSensorSubsetRes(cJSON *idsArray, configData *resData);
    void setSensorSelectionRes(cJSON *enabledArray, configData *resData);
    void setSensorMergeRes(cJSON *contentObj, configData *resData);
    void setSensorProtocolFailure(configData *resData, const char *reason);
    bool loadSensorCatalog(COLLECTMAP &catalog);
...
public:
    ...
    bool saveConfig(const char *path, const char *content);

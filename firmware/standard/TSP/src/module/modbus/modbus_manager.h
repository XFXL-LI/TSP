...
public:
    ...
    bool readModbusRegs(uint8_t slaveId, uint16_t startAddr, uint16_t count,
                        uint16_t* destBuffer, uint8_t maxAttempts = 3,
                        uint32_t responseTimeoutMs = 1000,
                        uint32_t retryDelayMs = 100);
    bool writeModbusRegs(uint8_t slaveId, uint16_t startAddr, uint16_t count,
                         uint16_t* dataBuffer, uint8_t maxAttempts = 3,

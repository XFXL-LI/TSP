#include "Pm25Collect.h"
#include "../../../../inc/sys_init.h"
#include "../../../../module/log/log_manager.h"
#include "../../../../module/Serial/SerialManager.h"
#include "../../collectorManager.h"
#include "../SensorReadPolicy.h"
static CollectorRegistrar _registrar_pm25("a34004", &Pm25Collect::getInstance());

Pm25Collect *Pm25Collect::_instance = nullptr;

Pm25Collect &Pm25Collect::getInstance()
{
    if (_instance == nullptr)
    {
        _instance = new Pm25Collect();
    }
    return *_instance;
}

Pm25Collect::Pm25Collect()
...
bool Pm25Collect::modbusInit(Stream* new_port, String id, String port_name, float factor, String unit)
{
    if (new_port == nullptr)
        return false;
    _port = new_port;
...
DataPacket *Pm25Collect::collect()
{
...
void Pm25Collect::setModbusConfig(uint8_t slave, uint16_t reg, uint8_t count, float factor)
{
    _slaveId = slave;
    _regAddr = reg;
    _regCount = count;
...
void Pm25Collect::setFactor(float factor)
{
    _factor = factor;
}

bool Pm25Collect::updatePort(Stream *new_port)
{
    if (new_port == nullptr)
        return false;
    _port = new_port;
...
void Pm25Collect::setIdentity(String id, int group)
{
    _id = id;
}

...
最后更新：2026-08-11
当前阶段：Firmware 2.0.5已基于8月10日验证版2.0.4完成优先级、日志和远程OTA整合并通过完整编译；尚未烧录或硬件验证，默认正式发布包仍为2.0.4

> 本文件是新会话的首要入口。新会话必须先完整读取本文件及
> `CHANGELOG_2.0.5.md`及`CHANGELOG_2.0.4.md`，先区分2.0.5编译状态与
> 2.0.4实机验证状态，再决定是否烧录或继续修改。

## 0. 2026-08-11 Firmware 2.0.5整合状态
...
- 构建状态为`compiled-not-hardware-verified`，本次没有烧录；

...
当前状态：Firmware 2.0.9 MCU协议已实现并编译通过，待发送端完成后实机联调

> 重要：Firmware 2.0.8及更早版本仍向Remote DTU发送英文文本`Ready`、字节
> 进度和成功/失败结果。Firmware 2.0.9只有在请求明确包含整数`protocol:1`时才
> 使用本文`upload_status` JSON；请求不带`protocol`时继续走legacy英文兼容路径。
> 2.0.9尚未烧录或与新版发送端实机联调，默认正式回退版本仍为Firmware 2.0.4。
...
[LCD OTA交互协议](../lcd/LCD_OTA_INTERACTION_PROTOCOL.md)
...
`630864` 只是 Firmware 2.0.9 当前编译 BIN 的示例大小。发送端必须读取用户实际
...
```json
{"operation":"upload_status","protocol":1,"session":1,"state":"verifying","written":630864,"total":630864,"progress":100}
```
...
```json
{"operation":"upload_status","protocol":1,"session":1,"state":"success","written":630864,"total":630864,"progress":100,"restarting_in_ms":3000}
```
...
```json
{"operation":"upload_status","protocol":1,"session":1,"state":"failed","reason":"image_validation_failed","written":630864,"total":630864,"progress":100}
```
...
以上数值只用于2.0.9当前编译包。该构建尚未实机验证；后续版本必须使用对应构建
清单中的大小和哈希，默认正式回退仍使用Firmware 2.0.4固定四段发布包。

## 12. 性能和 DTU 负担

本协议不会明显增加 DTU 负担：

- 630864字节、800字节一包约为789个发送包；
- 旧方案对每次写入返回文本，最多产生约789行进度；
...
## 13. Firmware 2.0.9 legacy旧文本兼容说明

Firmware 2.0.9收到不带`protocol`的旧请求时仍返回：

```text
Ready to start OTA, size: 630864 byte, inactivity timeout: 90 seconds
...
```text
OTA failed: image_validation_failed. Written 630864/630864 bytes.
```

旧发送端可以继续按本节工作；新发送端选择`protocol-v1`后只能解析JSON。不得在

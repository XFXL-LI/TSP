...
交接日期：2026-08-14
任务状态：可以开始发送端开发；等待 Firmware 2.0.9 提供新 JSON 协议实机联调版本
协议版本：1
...
### 2.2 Firmware 2.0.9 计划

MCU 计划在 Firmware 2.0.9 中单独实现 `upload_status` JSON。发送端可以先按本文和
正式协议完成开发、模拟测试和状态机测试，待 2.0.9 构建完成后进行实机联调。
...
| 模式 | 适用固件 | 启动 BIN 的依据 |
| --- | --- | --- |
| `legacy` | Firmware 2.0.8 及更早版本 | 现有英文 Ready 协议 |
| `protocol-v1` | 后续实现 `upload_status` 的固件 | 当前会话的 `state=ready` JSON |
...
其中 `627872` 只是当前 Firmware 2.0.8 应用 BIN 的示例大小。正式程序必须填入
用户实际选择文件的长度。
...
7. 等待 Firmware 2.0.9 构建后进行串口实机联调；
...
LCD OTA交互和normal恢复前置条件已经实机通过，Remote DTU发送端任务可以正式开始。
发送端先完成协议版本1的实现和模拟测试；新JSON端到端实机联调等待 Firmware 2.0.9。
